...
function module_webapp_getLabel()
{
    ...
    if module_webapp_isExist $1; then
        ...
        echo -n ${OLIX_MODULE_WEBAPP_LABEL}
    else
        echo -n "inconnu"
    fi
}
...
function module_webapp_saveFileConf()
{
    ...
    logger_debug "OLIX_MODULE_WEBAPP_LABEL=\"$(yaml_getConfig "label")\""
    echo "OLIX_MODULE_WEBAPP_LABEL=\"$(yaml_getConfig "label")\"" >> ${FILECONF}
    logger_debug "OLIX_MODULE_WEBAPP_FILEYML=${OLIX_MODULE_WEBAPP_FILEYML}"
    ...
    logger_debug "OLIX_MODULE_WEBAPP_ORIGIN_NAME=\"${OLIX_MODULE_WEBAPP_ORIGIN_NAME}\""
    echo "OLIX_MODULE_WEBAPP_ORIGIN_NAME=\"${OLIX_MODULE_WEBAPP_ORIGIN_NAME}\"" >> ${FILECONF}
    logger_debug "OLIX_MODULE_WEBAPP_ORIGIN_HOST=${OLIX_MODULE_WEBAPP_ORIGIN_HOST}"
    echo "OLIX_MODULE_WEBAPP_ORIGIN_HOST=${OLIX_MODULE_WEBAPP_ORIGIN_HOST}" >> ${FILECONF}
    logger_debug "OLIX_MODULE_WEBAPP_ORIGIN_PORT=${OLIX_MODULE_WEBAPP_ORIGIN_PORT}"
    echo "OLIX_MODULE_WEBAPP_ORIGIN_PORT=${OLIX_MODULE_WEBAPP_ORIGIN_PORT}" >> ${FILECONF}
    logger_debug "OLIX_MODULE_WEBAPP_ORIGIN_USER=${OLIX_MODULE_WEBAPP_ORIGIN_USER}"
    echo "OLIX_MODULE_WEBAPP_ORIGIN_USER=${OLIX_MODULE_WEBAPP_ORIGIN_USER}" >> ${FILECONF}
    logger_debug "OLIX_MODULE_WEBAPP_ORIGIN_PATH=${OLIX_MODULE_WEBAPP_ORIGIN_PATH}"
...
function module_webapp_loadConfiguration()
{
    ...
    source $(module_webapp_getFileConf $1)
    module_webapp_loadFileConfYML "${OLIX_MODULE_WEBAPP_FILEYML}" "$1"
}
...
function module_webapp_loadFileConfYML()
{
    ...
    yaml_parseFile "${FILECFG}" "${OLIX_MODULE_WEBAPP_CONFIG_PREFIX}"
    OLIX_MODULE_WEBAPP_CONFIG_DIR_WEBAPP=$(dirname ${FILECFG})
}
...
function module_webapp_setOrigin()
{
    ...
    OLIX_MODULE_WEBAPP_ORIGIN_NAME=$(yaml_getConfig "origin.server_${2}.name")
    OLIX_MODULE_WEBAPP_ORIGIN_HOST=$(yaml_getConfig "origin.server_${2}.host")
    OLIX_MODULE_WEBAPP_ORIGIN_PORT=$(yaml_getConfig "origin.server_${2}.port")
    OLIX_MODULE_WEBAPP_ORIGIN_USER=$(yaml_getConfig "origin.server_${2}.user")
    OLIX_MODULE_WEBAPP_ORIGIN_PATH=$(yaml_getConfig "origin.server_${2}.path")
    [[ -z ${OLIX_MODULE_WEBAPP_ORIGIN_HOST} ]] && logger_critical "Aucun serveur de dépot n'est déclaré dans le fichier webapp.yml"
